...
# ----------------------------------------------------
# 3. แผนกคลังสินค้าสำเร็จรูป (FG) - 🔥 ซ่อมระบบปุ่มเซฟล็อกเป้าหมาย JobID สำเร็จ 100%
# ----------------------------------------------------
    ...
    if fg_pending_list.empty:
        ...
        st.write("---")
        # 🚀 ปุ่มหลักแก้ไขสำเร็จ: บันทึกรับของเข้าคลัง FG แบบยกแผงปุ่มเดียว รันผ่านฉลุย ยอดวิ่งไปหน้า Admin ทันที!
        if st.button("💾 ยืนยันบันทึกรับสินค้าเข้าสต็อกทุกรายการที่เลือก (Approve ยกแผง)", type="primary", use_container_width=True):
            if fg_name.strip() == "":
                ...
                for option in selected_fg_jobs:
                    job_id_extracted = options_map_fg[option] # ดึง JobID ตรงตัว บล็อกคำสั่งไม่ระเบิด
# ----------------------------------------------------
